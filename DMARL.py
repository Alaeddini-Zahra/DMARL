...
for play in range(num_plays):
    ...
    for step in range(max_steps):
        ...
        # Update the Q-values with a game theory-based reward
        new_positions = agent_positions.copy()
        for agent_id in range(num_agents):
            x, y = new_positions[agent_id]
            reward = 0  # Initialize reward

            # Game theory reward strategy
            # Reward based on the cooperation or competition between agents
            if agent_id == 0:
                # Agent 0 tries to maximize the reward
                if (x, y) == (len(factors_1) - 1, len(factors_2) - 1):
                    reward = 1  # Positive reward for reaching the target
                elif (x + 1, y) == (len(factors_1) - 1, len(factors_2) - 1):
                    reward = 0.5  # Partial reward for moving closer to the target
                elif (x - 1, y) == (len(factors_1) - 1, len(factors_2) - 1):
                    reward = -0.5  # Negative reward for moving away from the target
            else:
                # Agent 1 tries to minimize the reward (competition)
                reward = -reward
